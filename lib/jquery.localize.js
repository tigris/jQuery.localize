...
(function() {
  ...
  $localize.nodeType = 'time';

  $localize.nodeAttr = 'datetime';
  ...
  jQuery.fn.localize = function(format) {
    ...
    this.each(function() {
      var $el, date, m;
      $el = jQuery(this);
      if (this.nodeName.toLowerCase() === $localize.nodeType) {
        m = re.exec($el.attr($localize.nodeAttr) || $localize('yyyy-mm-ddTHH:MM:ssZ'));
      }
      if (!m) {
        return;
      }
      date = normalize(new Date(Date.UTC(+m[1], m[2] - 1, +m[3], +m[4], +m[5], +m[6] || 0, +("" + (m[7] || 0) + "00").substr(0, 3))), m[8], m[9]);
      $el.attr($localize.nodeAttr, $localize(date, "yyyy-mm-ddTHH:MM" + (m[7] ? ':SS' : m[6] ? ':ss' : '') + "Z"));
      return handler.call($el, typeof format === 'function' ? format.call($el, date) : $localize(date, format));
    });
    options = {};
    return this;
  };
